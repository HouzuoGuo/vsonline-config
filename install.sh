...
sudo env DEBIAN_FRONTEND=noninteractive apt -q -y -f -m -o 'Dpkg::Options::=--force-confold' -o 'Dpkg::Options::=--force-confdef' install ansible apache2-utils apt-transport-https autoconf automake bash binutils busybox bzip2 caca-utils ca-certificates cgroup-tools cloc cscope ctags curl dateutils dbus diffutils dnsutils dos2unix expat findutils finger gdb gnupg gnutls-bin hostname htop iftop iotop iputils-ping iputils-tracepath lftp lm-sensors locales locales-all lrzsz lsof mailutils make minicom miscfiles moreutils mosh netcat net-tools nicstat nmap nmon openssl p7zip patchutils pciutils procps psmisc rsync screen shellcheck silversearcher-ag snmp socat software-properties-common strace sudo tcpdump tcptraceroute telnet tmux traceroute tree unar uniutils unzip usbutils util-linux util-linux-locales vim wbritish wget whois wiggle yamllint zip zlib1g
